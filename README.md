...
# XplagiaX — Beyond Xota · AI Detection & Forensic Integrity Microservice

**Version / Versión: `v3.0.0 "Beyond Xota"`** · Build `2026.07.24` · Engine: `desklib_v1` (DeBERTa-v3-large, single-model) · Segmenter: `segmentador_v1` (semantic topic-boundary) · Forensic reports: `v3.9` · License: proprietary — research use

> **v3.0.0 marks a core-architecture generation change** ("Beyond Xota"): the 2023-era 3-seed
...
**EN —** XplagiaX is a self-hosted forensic microservice that determines **which parts of a text were written by a human and which were generated by AI**. It does not return a single opaque score: it combines a single DeBERTa-v3-large neural detector (Desklib architecture) with **model-agnostic forensic signals** (fabricated-citation verification, discourse-structure analysis, internal-contradiction detection, authorship-signature breaks, perplexity) through a **bounded late-fusion verdict**, and produces evidence a human committee can verify — flagged paragraphs with full text, non-existent references, contradictory sentences, style-spliced sections. Text is split into topic-coherent segments by `segmentador_v1` (semantic topic-boundary detection, not fixed-length chunking) before scoring. Designed for theses, papers, and research documents up to **500,000 characters (~125K words)**, on CPU-only infrastructure, fully on-premise (no document ever leaves your servers).

**ES —** XplagiaX es un microservicio forense auto-hospedado que determina **qué partes de un texto fueron escritas por un humano y cuáles fueron generadas por IA**. No devuelve un número opaco: combina un único detector neuronal DeBERTa-v3-large (arquitectura Desklib) con **señales forenses model-agnósticas** (verificación de citas fabricadas, análisis de estructura del discurso, detección de contradicciones internas, rupturas de firma de autor, perplejidad) mediante un **veredicto de fusión tardía acotada**, y produce evidencia que un comité humano puede verificar: párrafos señalados con texto completo, referencias inexistentes, frases contradictorias, secciones con estilo injertado. El texto se divide en segmentos temáticamente coherentes mediante `segmentador_v1` (detección semántica de cambios de tema, no chunking de longitud fija) antes de puntuarlo. Diseñado para tesis, papers y documentos de investigación de hasta **500.000 caracteres (~125K palabras)**, en infraestructura solo-CPU, totalmente on-premise (ningún documento sale de tus servidores).
...
## 🆕 What's New — v3.0.0 "Beyond Xota" (July 2026) / Novedades

Core-engine generation change + scientific-audit phase 1 + thesis-scale processing. Full step-by-step SOTA roadmap in [`docs/sota/`](docs/sota/00_INDICE.md).

| ID | Area | Improvement (EN) | Mejora (ES) |
|---|---|---|---|
| BX-01 | Engine | **Detector swap**: the 3-seed `xota_ensemble` ModernBERT classifier (41-class softmax, per-family `detected_model`, `ensemble_disagreement`) is replaced by a single Desklib DeBERTa-v3-large binary detector ([app/engine/detector_final.py](app/engine/detector_final.py)) — mean-pooled hidden states → linear head → sigmoid P(AI). Same `DetectionResult` fields, same endpoints; `detected_model`/`ensemble_disagreement` are kept in the response for compatibility but are now always `null`/`0.0` (no per-family signal, no inter-seed variance to report — see [Engine](#engine--desklib-ai-text-detector)). | **Cambio de detector**: el clasificador ModernBERT de 3 semillas (`xota_ensemble`) es reemplazado por un único detector Desklib DeBERTa-v3-large — mismos campos de `DetectionResult`, mismos endpoints; `detected_model`/`ensemble_disagreement` se mantienen por compatibilidad pero ahora son siempre `null`/`0.0`. |
| BX-02 | Segmentation | **`segmentador_v1`** ([app/engine/segmentador.py](app/engine/segmentador.py)): topic-boundary semantic segmentation (TF-IDF + optional LSA) replaces fixed paragraph/sentence chunking for `analyze_fast()` — segment boundaries follow topic changes, not newline characters, so segmentation is identical whether or not the input has paragraph breaks. | **`segmentador_v1`**: segmentación semántica por cambio de tema (TF-IDF + LSA opcional) sustituye el chunking fijo por párrafo/oración — los cortes siguen el tema, no los saltos de línea. |
| BX-03 | Reliability | **Shared GPT-2 singleton**: `watermark_detection`'s entropy analyzer no longer loads its own independent GPT-2 copy — it now reuses `perplexity_check`'s Tier-2 GPT-2 singleton when already resident on the same device, eliminating a duplicate ~500 MB-1 GB model load that occurred whenever both plugins were active (the default). Falls back to its own independent load when Tier 2 is disabled — zero behavior change in that case. | **Singleton de GPT-2 compartido**: `watermark_detection` reutiliza el GPT-2 de `perplexity_check` (Tier 2) en vez de cargar su propia copia — elimina una duplicación real de memoria en la configuración por defecto. |
| BX-04 | Tech debt | **Dead-code audit across all 13 plugins + their engines**: ~1,000+ lines of confirmed-zero-caller code removed (`app/engine/legacy.py` in full, a 686-line abandoned authorship-comparison subsystem in `stylometric_profiler.py`, dead batch helpers, stale re-exports), plus C1-pattern duplicate-singleton fixes in `watermark_detection.py`/`zone_classifier.py`. Every removal verified via exhaustive repo-wide caller search before deletion — see [`docs/auditorias/`](docs/auditorias/) for the full evidence trail. | **Auditoría de código muerto en los 13 plugins**: 1000+ líneas eliminadas con evidencia exhaustiva de cero callers, más fixes de duplicación tipo C1. |
...
| F-02 | Fusion | Anti-overconfidence: neural log-odds capped ±4.0 + temperature prior 1.6 — kills the "falsely crisp 100%" audit finding. *(Historical/pre-BX-01: `ensemble_disagreement` was a 3-seed std OOD signal when the ModernBERT ensemble existed; the single Desklib model always reports it as `0.0` — see BX-01.)* | Anti-sobreconfianza: log-odds neuronal acotado ±4.0 + prior de temperatura 1.6 — elimina el "100% falsamente nítido" de la auditoría. *(Histórico/pre-BX-01: `ensemble_disagreement` medía desacuerdo entre 3 semillas; con el detector Desklib único siempre es `0.0`.)* |
...
| AC-01 | Correctness | *(Historical, pre-BX-01 — describes a bug in the now-replaced ModernBERT path, kept for the audit trail)* **`[CLS]`/`[SEP]` injection fix** ([detector_final.py](app/engine/detector_final.py)): `analyze_fast` fed ModernBERT content tokens with no `[CLS]` (a no-op `build_inputs_with_special_tokens`), **inverting every verdict** — known-human text scored 97% AI. Fixed by injecting `cls_token_id`/`sep_token_id` explicitly. See [AC-01](#fixes--improvements-july-2026--auditoría-científica-fase-1). | *(Histórico, pre-BX-01 — bug del path ModernBERT ya reemplazado, se conserva como registro de auditoría)* **Fix de inyección `[CLS]`/`[SEP]`**: el path `analyze_fast` alimentaba ModernBERT sin `[CLS]`, invirtiendo el veredicto — texto humano conocido daba 97% IA. |
...
│                                                                │
│  Master Process                                                │
│  ├── Desklib detector loaded ONCE at module level              │
│  ├── share_memory() → model in /dev/shm (shared across forks)  │
...
| `--shm-size=2g` (Docker) | Gives `/dev/shm` enough space for `share_memory()` to map the ~1.7 GB Desklib model into POSIX shared memory |
...
| `analyze_fast()` topic-boundary segmentation | `segmentador_v1` splits at semantic topic changes (TF-IDF/LSA), not raw token limits — prevents human/AI content bleeding across segment boundaries, identical result whether or not the input has paragraph breaks |

---

## Engine — Desklib AI-Text Detector

The core classifier ([app/engine/detector_final.py](app/engine/detector_final.py)) loads a single
...
| Aspect | Value |
|---|---|
| Backbone | DeBERTa-v3-large (`desklib/ai-text-detector-v1.01` by default) |
| Classifier head | Mean-pool over `attention_mask` → `nn.Linear(hidden_size, 1)` → `sigmoid` |
| Output | Single binary `P(AI)` — no per-family "which model wrote this" signal |
| Weight source (in order) | `DESKLIB_LOCAL_PATH` / `XPLAGIAX_EVAL_WEIGHTS` (local, offline) → `DESKLIB_MODEL_NAME` (HF Hub, default `desklib/ai-text-detector-v1.01`) → `MODEL_FALLBACK_DIR` |
| Provenance tracking | Every load attempt recorded in `_MODEL_LOAD_INFO`, surfaced via `get_model_info()` / [`/api/drift-status`](#get-apidrift-status) |

**Compatibility note:** `DetectionResult.detected_model` and `.ensemble_disagreement` are kept in
the dataclass and the JSON response for backward compatibility with existing consumers, but are
now always `None` / `0.0` — there is no ensemble to disagree and no per-family softmax to name a
generator from. `uncertainty_zone` is still computed, now purely from the human/AI margin
(`abs(ai% - human%) < 15`) rather than inter-seed variance.

The tokenizer and config are loaded from the local HuggingFace cache (or the Hub on first run —
baked into the Docker image at build time, see [Dockerfile](Dockerfile)).

### Segmentation — `segmentador_v1` (SegmentadorSemantico)

`analyze_fast()` ([app/engine/segmentador.py](app/engine/segmentador.py)) no longer splits text at
raw paragraph/sentence/token boundaries. Instead it builds **topic-coherent segments**:

1. Split into candidate sentences (title-aware, Markdown-aware).
2. Vectorize with TF-IDF, optionally reduce with LSA (SVD via `numpy.linalg.eigh`).
3. Detect topic-boundary discontinuities between adjacent sentence windows.
4. Merge sentences between boundaries into one segment — a segment ends where the *topic* changes,
   not where a newline happens to be.

This means the same underlying content is segmented identically whether it arrives as one
unbroken block of text or with paragraph breaks — segmentation reflects what the text is actually
about, not its formatting. Each segment is then scored independently by the Desklib detector via
`classify_batch()`, and results are cached in a shared thread-safe LRU (`_SEG_CACHE`, keyed by
`sha1(cleaned_text)`) — the SAME cache is consulted by both `analyze_fast()` (used by
`ai_detection`/`segment_analysis`) and `classify_batch()` (used by `HybridSegmentAnalyzer`), so
identical segment text scored via either path only pays for one forward pass.
...
|---|---|
| `ai_detection` | Binary Human/AI classification — single Desklib DeBERTa-v3-large detector. ~2s CPU, ~0.3s GPU. |
...
- **Qué hace (dummie):** un modelo entrenado (Desklib, DeBERTa-v3-large) lee el texto y estima
  qué porcentaje "huele" a escrito por IA. Es la huella estadística del estilo de escritura.
- **Cómo leerlo:** `ai_percentage` alto (>70) → sospecha de IA; `human_percentage` alto →
  sospecha de humano. **Mira siempre `uncertainty_zone`**: se activa cuando el margen entre
  `ai_percentage` y `human_percentage` es estrecho (<15 puntos) — en ese caso el % NO es confiable.
- **Limitación clave:** ningún clasificador supervisado es infalible contra modelos de frontera
  nuevos que no vio en entrenamiento. No te quedes solo con esto — usa `full_analysis` (§12).
- **Nota de compatibilidad:** `detected_model` y `ensemble_disagreement` siguen presentes en la
  respuesta (por compatibilidad con integraciones existentes) pero ahora son siempre `null`/`0.0`
  — eran señales del ensamble ModernBERT de 3 semillas (pre-v3.0.0); el detector Desklib es un
  único modelo binario sin variante "por familia de generador".
...
"data": {
  ...
  "uncertainty_zone": false,        // true = margen <15 puntos → no te fíes del %
  "ensemble_disagreement": 0.0,     // siempre 0.0 — histórico, ver nota de compatibilidad arriba
  "detected_model": null            // siempre null — histórico, ver nota de compatibilidad arriba
}
```
  ...
  "results": {
    "full_analysis": {
      ...
      "data": {
        ...
        "detection": {                   // ALWAYS — Desklib neural verdict
          "prediction": "Human | AI | Unknown",
          "confidence": 0.0,
          "human_percentage": 50.0,
          "ai_percentage": 50.0,
          "detected_model": null,        // always null — single-model detector, see Engine section
...
```json
{
    ...
    "model": {
        ...
        "device": "cpu",
        "weights": [
            {"requested": "desklib/ai-text-detector-v1.01", "loaded_from": "huggingface_hub_cache", "fallback": false}
        ],
        "fallbacks_used": []
    }
}
...
2. **`"Inconclusive"` is a feature, not an error.** It fires on: <200 words, gray band,
   code-heavy documents, heavily quoted documents, or contradictory evidence.
   `verdict_uncertainty.reason` says which. Treat it as "route to a human". / Es una
   abstención deliberada, no un fallo.
...
python scripts/retrain_pipeline.py promote  --corpus data/gold \
    --weights /path/new/model_dir --version 2026.08 --min-gain 0.01
```

> ⚠️ **Known gap:** `scripts/retrain_pipeline.py`'s `promote` step still assumes the pre-v3.0.0
> single-file ModernBERT weight format (`--weights <file>.bin`, seed-12/seed-22 naming). It has
> not yet been updated for Desklib's `from_pretrained(directory)` loading convention — treat the
> neural-promotion layer above as **not yet validated** against the new detector until this
> script is updated. Layer A (fusion training) is architecture-independent and unaffected.
...
| `{"status": "error", "error": "Desklib AI detector not loaded..."}` | Model weights unavailable — check `DESKLIB_MODEL_NAME`/`DESKLIB_LOCAL_PATH` |
...
```json
{
    ...
    "results": {
        "ai_detection": {
            "status": "error",
            "error": "Desklib AI detector not loaded. Check model paths.",
            "elapsed_ms": 0.3
        }
    }
}
...
# 3. Cache the Desklib detector locally (downloads once, then reused from the
#    HuggingFace cache — set DESKLIB_LOCAL_PATH instead to load fully offline
#    from a local snapshot directory)
python -c "from transformers import AutoTokenizer, AutoConfig, AutoModel; \
name='desklib/ai-text-detector-v1.01'; AutoConfig.from_pretrained(name); \
...
|---|---|
| `--shm-size=2g` | Docker asigna 64 MB a `/dev/shm` por defecto. `m.share_memory()` necesita ~1.8 GB para el modelo Desklib. Sin este flag, el módulo falla al importar y el modelo no se carga. |
| `--memory=6g` | La app necesita ~4.4 GB en carga inicial (2 GB shmem + 2 GB anon). Sin límite suficiente, el OOM killer mata gunicorn antes de que cargue el modelo. |
...
#### Run — with a mounted model snapshot (avoids embedding ~1.7 GB in the image)
...
docker run -d \
  ...
  --restart unless-stopped \
  -p 5006:5006 \
  -v /path/to/desklib-snapshot:/models/desklib \
  -e DESKLIB_LOCAL_PATH=/models/desklib \
  -e WEB_CONCURRENCY=2 \
  -e REDIS_URL="redis://redis:6379" \
  -e CROSSREF_EMAIL="your@institution.edu" \
  xplagiax_xota:latest
...
> **Note**: `preload_app=True` ensures the ~1.7 GB Desklib model is loaded once in the Gunicorn master process and shared across all workers via Linux Copy-on-Write. Each worker adds only ~50 MB overhead instead of another full model copy.
...
| `DESKLIB_MODEL_NAME` | `desklib/ai-text-detector-v1.01` | HuggingFace Hub repo id for the detector — override to pin a different Desklib checkpoint |
| `DESKLIB_LOCAL_PATH` / `XPLAGIAX_EVAL_WEIGHTS` | _(none)_ | Local model snapshot directory — loads fully offline (`local_files_only=True`), tried before the Hub id |
| `MODEL_FALLBACK_DIR` | _(none)_ | Last-resort local snapshot if both the primary source and Hub download fail |
| `DESKLIB_MAX_LEN` | `768` | Max tokens per forward pass through the Desklib classifier |
...
│       ├── detector_final.py     # Desklib single-model detector + analyze_fast() (length-bucketed batching) + TTL cache
│       ├── segmentador.py        # segmentador_v1 — TF-IDF/LSA topic-boundary semantic segmenter
...
│           # (no model weight files checked in — Desklib is fetched into the
│           #  HuggingFace cache at Docker build time or on first run; see Dockerfile)
│
├── docs/
│   ├── EXPERIMENTAL_PROTOCOL.md  # Retraining / evaluation protocol
│   ├── RETRAINING_MANUAL.md      # Bilingual full retraining operational manual
│   ├── auditorias/               # Historical audit & analysis reports (see below)
│   ├── md/                       # Reference papers (AI-text detection, hallucination, stylometry)
...
└── tests/
    ...
    ├── test_model_shapes.py      # Desklib classifier head shape tests (gated: RUN_MODEL_SMOKE=1)
    ├── test_precision_corpus.py  # Rolling-corpus precision regression tests
    ├── test_drift_monitor.py     # /api/drift-status monitor tests
    ├── test_fase2_pipeline.py    # Scientific-audit Fase 2 pipeline tests
    ├── test_lang_semantic.py     # Language-gated semantic-consistency tests
    ├── test_registry_deadline.py # Plugin registry deadline/timeout tests
    ├── test_plugins.py           # Plugin registry / auth tests
    └── test_routes.py            # API endpoint tests
```

`docs/auditorias/` holds the project's historical audit trail — scientific audits, plugin
committee reviews, and technical-debt analyses (`AUDITORIA_CIENTIFICA.md`,
`AUDITORIA_CIENTIFICA_FASE2.md`, `AUDITORIA_PLUGINS.md`, `AUDITORIA_TECNICA.md`,
`auditoria_tecnica_xplagiax.md`, `ENSHITTIFICATION_ANALYSIS.md`,
`ENSHITTIFICATION_IMPLEMENTATION.md`, `observacion.md`, `para_superar_a_la_competencia.md`,
`posibles_mejoras.md`) — kept for evidence/provenance, not day-to-day reading.
...
Running a separate Celery worker container duplicates the ~1.7 GB Desklib model in memory. Instead, the application uses a **single container** where Gunicorn's master process loads the model once (`preload_app=True`) and internally forks the Celery worker (`when_ready` hook).
* **Result:** Total memory usage drops from ~6.8 GB to ~2.8 GB because the model is shared via Linux Copy-on-Write (CoW).
...
curl -X POST http://localhost:5006/analyze_document_async \
...
> **Historical, pre-BX-01.** This analysis targeted the 3-seed ModernBERT ensemble
> (3 × ~570 MB weight files). Since v3.0.0 there is one Desklib model (~1.7 GB total,
> same order of magnitude) instead of three ~570 MB files — the per-file breakdown below
> no longer matches `app/engine/`, but the CoW/Celery/Redis root causes and fixes below it
> are architecture-independent and still apply as documented.
...
### Baseline 2.7 GB — Decomposition (historical, 3-model ensemble)
...
| Proceso | RAM |
|---|---|
| Modelo Desklib en `/dev/shm` (compartido) | ~2.0 GB |
...
> `initialDelaySeconds` is set high because the ~1.7 GB Desklib model takes ~20-40s (up to ~60s on slow disks) to load on first startup.
